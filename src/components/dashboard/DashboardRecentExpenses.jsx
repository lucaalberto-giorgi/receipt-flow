...
function DashboardRecentExpenses({ expenses }) {
  return (
    <section className="rounded-[28px] border border-violet-100/80 bg-white p-5 shadow-[0_24px_48px_-38px_rgba(15,23,42,0.35)] sm:p-6">
      <div className="flex items-center justify-between gap-4 border-b border-violet-100/80 pb-5">
        <div>
        ...
        <span className="rounded-2xl border border-violet-100 bg-violet-50 px-3 py-2 text-sm text-violet-700">
      ...
      <div className="mt-4 space-y-3">
        {expenses.map((expense) => (
          <article
            key={expense.id}
            className="flex flex-col gap-4 rounded-[24px] border border-violet-100/80 bg-violet-50/35 p-4 sm:flex-row sm:items-center sm:justify-between"
          >
            <div>
            ...
            <div className="flex flex-wrap items-center gap-3 sm:justify-end">
              <span className="inline-flex rounded-full border border-violet-100 bg-white px-3 py-1 text-xs font-medium text-violet-700">
                {expense.category}
              </span>
